...
data_file = st.file_uploader("📊 Charger le fichier de données (.xlsx ou .fit)", type=["xlsx", "fit"])

remarques = st.text_area("🖊️ Remarques", "Bonne stabilité jusqu’à S2, réoxygénation rapide post-PMA.")

if txt_file and data_file:
    identity = {}
    ...
    df = charger_donnees(data_file, extension).dropna()
