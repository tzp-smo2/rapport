...
def generer_rapport(xlsx_file, txt_file, logo_path="logo_tzp.png", dossier_sortie="."):
    ...
    zones = {
        "Zone 1 (récupération)": f"< {0.85 * x0:.0f} s",
        "Zone 2 (aérobie basse)": f"{0.85 * x0:.0f} - {x0:.0f} s",
        "Zone 3 (aérobie haute)": f"{x0:.0f} - {x0_2:.0f} s",
        "Zone 4 (seuil anaérobie)": f"{x0_2:.0f} - {1.10 * x0_2:.0f} s",
        "Zone 5 (VO2max)": f"> {1.10 * x0_2:.0f} s"
    }
    ...
    plt.plot(x, piecewise_linear(x, *params1), 'r', label="Modèle S1")
    plt.axvline(x=x0, color='green', linestyle='--', label=f"S1 : {x0:.1f} s")
    plt.axvline(x=x0_2, color='blue', linestyle='--', label=f"S2 : {x0_2:.1f} s")
    ...
    c.setFont("Helvetica", 10)
    current_y -= 0.6 * cm
    c.drawString(margin, current_y, f"{x0:.1f} s | SmO₂ : {y0:.1f}% | HR : {hr_seuil1} bpm")
    ...
    c.setFont("Helvetica", 10)
    current_y -= 0.6 * cm
    c.drawString(margin, current_y, f"{x0_2:.1f} s | SmO₂ : {y0_2:.1f}% | HR : {hr_seuil2} bpm")
