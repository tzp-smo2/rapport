...
def generer_rapport(xlsx_file, txt_file, logo_path="logo_tzp.png", dossier_sortie="."):
    ...
    possible_power_cols = ["Power[W]", "Power - 2[W]", "Target[w]", "Target[W]", "Target Pwr[W]"]
    possible_smo2_cols = ["SmO2[%]", "SmO2 - 2[%]", "SmO2 - 3[%]"]
    possible_hr_cols = ["HR[bpm]", "HeartRate[bpm]"]

    col_power = next((c for c in df.columns if c.strip() in possible_power_cols), None)
    col_smo2 = next((c for c in df.columns if c.strip() in possible_smo2_cols), None)
    col_hr = next((c for c in df.columns if c.strip() in possible_hr_cols), None)

    if not col_power:
        raise ValueError("Colonne de puissance non trouvée. Attendu : Power[W], Power - 2[W] ou Target[w]")
    if not col_smo2:
        raise ValueError("Colonne SmO₂ non trouvée. Attendu : SmO2[%], SmO2 - 2[%] ou SmO2 - 3[%]")
    if not col_hr:
        raise ValueError("Colonne de fréquence cardiaque non trouvée.")
    ...
    x = df_clean["Time[s]"].values  # Remplacement : abscisse = temps
    ...
    plt.title("SmO₂ vs Temps - Courbe lissée", fontsize=9)
