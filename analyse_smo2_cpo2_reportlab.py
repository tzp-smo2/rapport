...
def generate_pdf(output_path, identity, seuils, zones, remarques, graph_path, logo_path):
    ...
    styleH = ParagraphStyle(name='Titre', parent=styles['Heading1'], fontName='DejaVu', alignment=TA_LEFT, fontSize=16)
    ...
    # En-tête avec titre + logo flottant
    titre = Paragraph("Rapport de Test SmO₂ – CPO2", styleH)
    if logo_path and os.path.exists(logo_path):
        logo_img = ImageReader(logo_path)
        iw, ih = logo_img.getSize()
        target_height = 3.5 * cm
        aspect = iw / ih
        logo = Image(logo_path, width=target_height * aspect, height=target_height)
        logo.hAlign = 'RIGHT'
        title_table = Table([[titre, logo]], colWidths=[None, 4*cm])
    else:
    ...
    # Graphique centré (si disponible)
