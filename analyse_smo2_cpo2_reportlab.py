...
def generate_pdf(output_path, identity, seuils, zones, remarques, graph_path, logo_path):
    ...
    styleH = ParagraphStyle(name='Titre', parent=styles['Heading1'], fontName='DejaVu',
                            alignment=TA_LEFT, fontSize=16, spaceAfter=6, leading=18)
    styleH.fontSize = 16
    styleH.leading = 18
    ...
    # En-tête avec titre + logo flottant (logo max 5cm, titre aligné en haut avec retour ligne)
    ...
    if logo_path and os.path.exists(logo_path):
        ...
        logo.hAlign = 'RIGHT'
        title_table = Table([[titre, logo]], colWidths=[16*cm - logo_width, logo_width])
        title_table.setStyle(TableStyle([('VALIGN', (0, 0), (0, 0), 'TOP')]))
    else:
